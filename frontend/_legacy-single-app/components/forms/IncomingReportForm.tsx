...
export const IncomingReportForm: React.FC = () => {
  ...
  const onSubmit = (data: IncomingReportFormData) => {
    ...
    mutate(data, {
      onSuccess: () => {
        setSuccessMessage(`Incoming report for "${data.materialName}" (${data.quantity} ${weightUnit}) recorded successfully! Broadcast sent to Live Dashboard.`);
        reset({
          materialName: '',
          quantity: undefined,
          price: undefined,
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-4">
        ...
        <div className="p-4 rounded-2xl bg-[#f8faf9] border border-[#d2ded8] flex items-center justify-between shadow-2xs">
          <div className="flex items-center gap-2 text-xs font-semibold text-[#71817E]">
            ...
            <span>Computed Inward Total ({weightUnit}):</span>
          </div>
