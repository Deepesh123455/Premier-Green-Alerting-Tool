...
export const PremiumClockPicker: React.FC<PremiumClockPickerProps> = ({
  ...
  // Format readable display: e.g. "01:25 PM IST"
  ...
  return (
    <div ref={containerRef} className="relative w-full flex flex-col space-y-1.5">
      ...
      <button
        ...
        className={clsx(
          'w-full px-4 py-3 bg-white text-left border rounded-xl text-sm transition-all flex items-center justify-between shadow-xs',
          isOpen
      ...
      {isOpen && (
        <div className="absolute top-full left-0 z-50 mt-1.5 w-72 bg-white border border-[#d2ded8] rounded-3xl shadow-2xl p-5 animate-in fade-in zoom-in-95 duration-150">
          ...
          <div className="relative w-[210px] h-[210px] mx-auto bg-[#f8faf9] border border-[#d2ded8] rounded-full shadow-inner flex items-center justify-center select-none">
            ...
            {/* Animated Clock Hand Pointer */}
            <div
              className="absolute w-1 bg-[#234D42] origin-bottom transition-all duration-200 pointer-events-none z-10"
              style={{
                height: `${radius - 12}px`,
                bottom: '50%',
                left: 'calc(50% - 2px)',
              ...
              hoursList.map((hour, index) => {
                ...
                return (
                  <button
                    ...
                    className={clsx(
                      'absolute w-7 h-7 rounded-full font-bold text-xs flex items-center justify-center transition-all z-20 cursor-pointer',
                      isSelected
              ...
              minutesList.map((minute, index) => {
                ...
                return (
                  <button
                    ...
                    style={{ left: `${x}px`, top: `${y}px` }}
                    className={clsx(
                      'absolute w-7 h-7 rounded-full font-bold text-[11px] font-mono flex items-center justify-center transition-all z-20 cursor-pointer',
                      isSelected
                        ? 'text-white font-black'
                        : 'text-[#1a2522] hover:bg-[#e9f2ef]'
                    )}
          ...
          <div className="mt-4 pt-3 border-t border-[#eef2f0] flex items-center justify-between text-xs">
            <button
              ...
              className="font-bold text-[#234D42] hover:underline"
            >
              Current Time
            </button>
            <button
              ...
              className="font-bold bg-[#234D42] text-white px-3.5 py-1.5 rounded-xl shadow-xs hover:bg-[#1a3b32] transition-colors"
            >
              Done
            </button>
          </div>
